...
<body>
    ...
    <div class="container" id="ps">
      ...
      <div class="profile-stats">
        ...
        <div>
          <span style="text-decoration: underline;">過往 & 評價</span>
        </div>
      </div>
      ...
      <?php 
        class postOver {
          ...
          public function __construct($id, $sport, $location, $number, $time, $picsrc){
            $this->id = $id;
            $this->sport = $sport;
        ...
        $sql = "SELECT DISTINCT event.event_id AS 'id', event.sport AS 'sport', event.location AS 'location', event.people_Needed AS 'number', event.event_Time AS 'time', user.gender AS 'gender' FROM event LEFT JOIN queue ON queue.event_id = event.event_id 
          LEFT JOIN user ON user.user_id = event.user_id
          WHERE event.state = 'success' AND (event.user_id = '$userId' OR (queue.waiting_id = '$userId' AND queue.state = 'picked')) ORDER BY event.event_Time DESC";
        ...
        while($row = mysqli_fetch_assoc($result)){
          ...
          $obj = new postOver($id, $sport, $location, $number, $eventTime, $picsrc);
          $postsOver[] = $obj;
        }
      ...
      <div>
        <script>
          ...
          notYet.textContent = "您  目  前  還  沒  有  評  價";
          notYetCon.appendChild(notYet);

          var notYet2 = document.createElement('h3');
          notYet2.textContent = "快   去   參   加   活   動   吧 ！";
          ...
          for (var i = 0; i < postsData.length; i++){
            ...
            buttonElement.onclick = function() {
              var btnId = this.id;
              location.href = "ratingPage.php?id=" + btnId;
              
              console.log(btnId);
              // Create an AJAX request
              var xhr = new XMLHttpRequest();
              xhr.open('GET', 'ratingPage.php?id=' + btnId, true);
