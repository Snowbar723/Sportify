...
<head>
  ...
  <title><?php echo($nickname) ?> 的個人頁面</title>
  <style>
  body {
...
<body>
    ...
    <div class="container" id="ps">
      ...
      <div class="profile-stats">
        <div>
          <span style="text-align:center; text-decoration: underline; display:block">過往 & 評價</span>
        </div>
      </div>
...
<?php 
        class postOver {
          ...
          public function __construct($id, $sport, $location, $number, $time, $user){
            $this->id = $id;
            $this->sport = $sport;
        ...
        $sql = "SELECT DISTINCT event.event_id AS 'id', event.sport AS 'sport', event.location AS 'location', event.people_Needed AS 'number', event.event_Time AS 'time' FROM event LEFT JOIN queue ON queue.event_id = event.event_id WHERE event.state = 'success' AND (event.user_id = '$userId' OR (queue.waiting_id = '$userId' AND queue.state = 'picked')) ORDER BY event.event_Time DESC";
        ...
        while($row = mysqli_fetch_assoc($result)){
          ...
          $obj = new postOver($id, $sport, $location, $number, $eventTime, $userId);
          $postsOver[] = $obj;
        }
      ...
      <div>
        <script>
          ...
          notYet.textContent = "他  目  前  還  沒  有  評  價";
          ...
          for (var i = 0; i < postsData.length; i++){
            ...
            buttonElement.onclick = function() {
              var btnId = this.id;
              location.href = "ratingPage.php?id=" + btnId + '&name=' + post.user;
              
              console.log(btnId);
              // Create an AJAX request
              var xhr = new XMLHttpRequest();
              var link = 'ratingPage.php?id=' + btnId + '&name=' + post.user;
              console.log(link);
